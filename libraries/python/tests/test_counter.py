...
def test_create_counter_cache(client):
    responses.add(
        responses.POST,
        "http://localhost:7070/api/v1/counters/",
...
def test_put_counter(client):
    responses.add(
        responses.POST,
        "http://localhost:7070/api/v1/counters/test-counter",
        json={"cacheId": "test-counter", "key": "hits", "operationStatus": "SUCCESS"},
        status=200
    )
...
def test_get_counter(client):
    responses.add(
        responses.GET,
        "http://localhost:7070/api/v1/counters/test-counter/hits",
        json={"cacheId": "test-counter", "key": "hits", "value": 42, "operationStatus": "SUCCESS"},
        status=200
    )
...
def test_increment_counter(client):
    responses.add(
        responses.POST,
        "http://localhost:7070/api/v1/counters/increment/test-counter",
        json={"cacheId": "test-counter", "key": "hits", "value": 43, "operationStatus": "SUCCESS"},
        status=200
    )
...
def test_decrement_counter(client):
    responses.add(
        responses.POST,
        "http://localhost:7070/api/v1/counters/decrement/test-counter",
        json={"cacheId": "test-counter", "key": "hits", "value": 41, "operationStatus": "SUCCESS"},
        status=200
    )
...
def test_set_counter(client):
    responses.add(
        responses.POST,
        "http://localhost:7070/api/v1/counters/set/test-counter",
        json={"cacheId": "test-counter", "key": "hits", "value": 100, "operationStatus": "SUCCESS"},
        status=200
    )
...
def test_bulk_ops(client):
    ...
    # The serialized request should omit unset (None) fields entirely.
    sent_body = responses.calls[0].request.body
    if isinstance(sent_body, bytes):
        sent_body = sent_body.decode()
    assert "null" not in sent_body
    assert "ttl" not in sent_body

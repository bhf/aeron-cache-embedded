...
// As with the string cache, we test EmbeddedCounterCache against a mock HTTP server,
// verifying that its methods route through the client to the correct counter endpoints.
...
fn test_embedded_counter_remove_and_clear_delegate() {
    ...
    let del_mock = server.mock("DELETE", "/api/v1/counters/test-counter/hits")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{"cacheId": "test-counter", "key": "hits"}"#)
        .create();

    let clear_mock = server.mock("DELETE", "/api/v1/counters/test-counter")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{"cacheId": "test-counter"}"#)
        .create();
